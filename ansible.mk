...
ansible.provision: clean ansible.update
	@ansible-playbook -vvvv "${ANSIBLE_DIR}/${env}/provision.yaml" -i "${ANSIBLE_DIR}/${env}/inventories/aws"  -v \
					--user="${USER}"  --private-key="${KEYS_DIR}/${PROJECT}-${env}.pem" \
					--extra-vars "@${ANSIBLE_DIR}/${env}/vars/vars.yaml" \
					--vault-password-file "${HOME}/${PROJECT}-${env}.txt"

ansible.deploy: clean ansible.update
	@ansible-playbook -vvvv "${ANSIBLE_DIR}/${env}/deploy.yaml" -i "${ANSIBLE_DIR}/${env}/inventories/aws"  -v \
					--user="${USER}"  --private-key="${KEYS_DIR}/${PROJECT}-${env}.pem" \
					--extra-vars "@${ANSIBLE_DIR}/${env}/vars/vars.yaml" \
					--vault-password-file "${HOME}/${PROJECT}-${env}.txt"
